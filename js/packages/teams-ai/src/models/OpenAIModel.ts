    ...
    protected createChatCompletion(request: CreateChatCompletionRequest, model: string): Promise<AxiosResponse<CreateChatCompletionResponse>> {
        if (this._useAzure) {
            ...
            const url = options.dataSources
                ? `${options.azureEndpoint}/openai/deployments/${model}/extensions/chat/completions?api-version=${options.azureApiVersion!}`
                : `${options.azureEndpoint}/openai/deployments/${model}/chat/completions?api-version=${options.azureApiVersion!}`;
            return this.post(url, request);
        } else {
            const options = this.options as OpenAIModelOptions;
